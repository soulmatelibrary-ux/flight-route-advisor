...
> ✅ **docs 11~14는 "AI 경로추천 근거화" 추가개발 묶음 — Phase A·B·C(STEP A1~A7·B1~B2·C1~C4) 전부
> 완료(2026-07-24)**. 남은 것은 모드 C 실제 활성화(제공자/모델 확정 필요, [13 §12](docs/13-ai-reasoning-dev-plan.md))뿐. 읽는 순서: **12(왜) → 11(분석) → 14(확정 결정) → 13(구현 STEP·진행상태 단일출처)**.
...
구현 단위([docs/07](docs/07-checklist.md)의 각 항목, 예: B1·B2·B-exit)를 마칠 때마다 **리뷰에이전트**(`code-reviewer`/`senior-code-reviewer`)로 다음을 점검하고, **지적사항을 고친 뒤에만** 체크한다.
1. 기능 버그  2. 논리 버그  3. 예외처리 누락  4. 보안 취약점(시큐어코딩 §3 위반)

**여러 항목을 묶어서 한 번에 리뷰하지 않는다.** B1을 끝냈으면 B2로 넘어가기 전에 B1부터 리뷰·수정을 마친다. 하위 항목 하나 = 리뷰 사이클 하나.
