...
| ② 공항 기상 카테고리 | 출·도착점 VFR/MVFR/IFR/LIFR 색 뱃지 + 클릭 시 METAR/TAF 팝업 | 항상 | MVP(기존 기상서버, [03 §5](./03-backend-api.md)) |
| ③ 경유 FIR 심각도 | 포커스 FIR 면에 강수·위험 틴트(경유 FIR만, 전세계 아님) | 항상 | MVP(RainViewer 집계) |
| ④ 위험기상(SIGMET/PIREP) | 근거 패널 칩(TS·ICE·TURB) + 루트 교차 시 지도 경고 마커 | 조건부 | MVP(기존 기상서버 `isigmet`/`pirep`, [03 §5](./03-backend-api.md)) |
| ⑤ 기상 레이더(RainViewer) | 토글 레이어 + 재생 바(과거 2h). 결정 포커스 기본 OFF, 온디맨드 | 토글 | MVP |

- MVP(①제외)는 기존 Node 기상서버·RainViewer·ADS-B — **신규 백엔드 개발 없이** 브라우저 직접/프록시 호출로 충족([03 §5](./03-backend-api.md)). ①(시어·상층풍·추천고도)은 Open-Meteo 연동·세그먼트 계산이 필요한 04-E 작업이므로 MVP 화면에는 **자리를 만들지 않고 2단계 착수 시 추가**한다.
- 데이터 출처: 원본 `문서/03·04·07`(공항 기상=기상서버 METAR/TAF, 시어·상층풍=Open-Meteo[2단계], 레이더=RainViewer, SIGMET/PIREP=기상서버 `isigmet`/`pirep`). 외부 API 폴백·CORS 규약은 [06](./06-conventions.md) 유지.
- 우측 근거 패널: MVP는 "경유 FIR 체인"만(§2.2). 배풍/정풍·측풍·시어 등급·★추천 고도(①)와 지연원인(FOIS)·흐름관리 상태는 2단계 진입 후 추가(03 §4.2~4.4).
