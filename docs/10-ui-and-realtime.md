...
│         · FOIS/흐름관리/ACDM 조회 아이콘(우측)                │   ← 2026-07-24, 아래 "수동 조회 3종" 참고
├───────────────┬──────────────────────────┬─────────────────┤
│ 결정 패널(좌)  │      포커스 맵(중앙)       │ 의사결정 근거(우) │
│ · 출발/도착    │  선택 FIR·루트만, fit    │ · 경유 FIR 체인   │   ← MVP(ODR2 enroute_firs)
│ · 후보 루트    │  bounds, 루트 강조        │ · 상층풍/시어     │   ← 2단계(04-E)
│   (편수·평균   │                          │ · 지연원인(FOIS) │   ← 2단계(03 §4.3), 노선 선택 시 자동 요약만
│    소요·지연수)│                          │ · 흐름관리 영향   │   ← 2단계(03 §4.4), 노선 선택 시 자동 요약만
│ · 필터         │                          │                  │
├───────────────┴──────────────────────────┴─────────────────┤
...
> **수동 조회 3종(FOIS/흐름관리/ACDM)의 UI 위치 변경(사용자 요청, 2026-07-24)**: 세 폼은 원래 우측 패널(280px 고정폭)에 항상 펼쳐져 있었으나, 이미 이 저장소가 겪은 것과 같은 문제(폭 좁아 날짜입력·목록이 줄바꿈으로 뭉개짐, `route-detail-drawer` 도입 배경과 동일)가 재현돼 사용자가 스크린샷으로 지적했다. 검토 후 **상단바 아이콘 버튼 + 중앙 팝업(`<dialog>`)** 방식으로 전환(기존 `#reasoning-dialog`와 동일 메커니즘, 폭 제약 없음) — 대안이었던 "우측 패널 접기/펼치기"는 폭 제약 자체를 해결하지 못해 채택 안 함. 우측 패널은 이제 노선 선택 시 자동 채워지는 "의사결정 근거" 요약(FIR 체인·정시율/지연원인/흐름관리 라이트 연동)만 남고, 세 폼은 각각 독립된 조회 도구로 분리됐다(`frontend/js/query-dialogs.js`, 조회 로직 자체는 무변경).
