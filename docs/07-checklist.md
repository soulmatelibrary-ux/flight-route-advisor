...
- [ ] **(2026-07-21 후속 리뷰, 미해결)** 업로드 포맷 확장(45번)·조회/다운로드(47번) 항목에 대해 `senior-code-reviewer` 2차 리뷰 진행, 아래 지적사항은 아직 수정 전:
  - **[Medium/논리]** `loaders.py`(FOIS raw 읽기): xlsx 원본은 항상 버려지는 1행이 있어 `skiprows=1`을 쓰는데, 신규 허용된 FOIS CSV 경로에도 같은 가정을 검증 없이 적용함. 실제 데이터부터 시작하는 CSV가 들어오면 첫 데이터 행이 조용히 유실되고 열 수 검증(17열)은 우연히 통과할 수 있음 — 예외 없이 성공으로 위장하는 유일한 케이스라 우선순위 높임. 실제 FOIS CSV 샘플 확보 후 가정 검증 또는 값 기반 스킵 판별 필요
  - **[Medium/기능완결성]** `workspace_builder.py`/`loaders.py`의 신규 CSV 경로 인코딩이 전부 `utf-8-sig` 하드코딩 — CP949/EUC-KR 원본(레거시 관제 시스템에서 흔함)은 새로 연 CSV 업로드 기능 자체가 전면 실패. 실패 시 오류 자체는 안전하게 처리되나(내부정보 노출 없음), 인코딩 자동감지/폴백 부재로 기능 실효성이 떨어짐
  - **[Low/예외처리]** `routers/runs.py`의 `hmac.compare_digest(admin_token, settings.delete_token)` — 두 값 중 하나라도 비-ASCII면 `TypeError`(값이 다르다는 안전한 `False`가 아님)로 처리되지 않은 500이 날 수 있음. 토큰 ASCII 전용 문서화 또는 비교 전 예외 처리 필요
  - **[Low/성능]** `download_data_csv`/`run_data`(신규 조회·다운로드 엔드포인트)가 청크마다 커넥션을 새로 열고 `OFFSET` 기반 페이지네이션 사용 — run 데이터가 대량으로 누적된 뒤 다운로드 후반부 청크로 갈수록 느려지는 전형적 OFFSET 비용 발생. 키셋 페이지네이션(`WHERE id > :last_id`)으로 전환 권장
  - 반대로 사용자가 사전에 의심했던 인증 부재·삭제 트랜잭션 경합·삭제 대상 테이블 완전성·xlrd 보안·삭제 파일 경로주입 5개 항목은 이번 라운드에서 전부 문제없음/이미 해소로 확인됨(`INGESTION_DELETE_TOKEN` fail-closed 게이트 확인)
