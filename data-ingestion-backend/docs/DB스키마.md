...
| `processed_flow_management` | `흐름관리ID, 원본순번, 기록일자, 통보시각, 적용시작일시, 적용종료일시, 적용분, 익일시작보정, 발신기관, 수신기관, 사유코드, 기상세부분류, MINIT, MIT, 고도속도제한, 원본대상, 원본항공로, 원본지점, 대상공항, 대상FIR, 대상항공로, 대상지점, 제외공항, 특수범위, 공간범위유형, 방향, 비고, CTOT비고, 제한내용요약, 품질상태` |
...
### 9.6 `processed_flow_management` (논리 30열)

| 논리명 | 물리 컬럼 | 논리명 | 물리 컬럼 |
|---|---|---|---|
| 흐름관리ID | `flow_id` | 원본대상 | `raw_destination` |
| 원본순번 | `source_seq` | 원본항공로 | `raw_route` |
| 기록일자 | `record_date` | 원본지점 | `raw_fix` |
| 통보시각 | `notify_time` | 대상공항 | `target_airport` |
| 적용시작일시 | `apply_start_dt` | 대상FIR | `target_fir` |
| 적용종료일시 | `apply_end_dt` | 대상항공로 | `target_route` |
| 적용분 | `apply_minutes` | 대상지점 | `target_fix` |
| 익일시작보정 | `next_day_adjusted` | 제외공항 | `excluded_airport` |
| 발신기관 | `sender` | 특수범위 | `special_scope` |
| 수신기관 | `receiver` | 공간범위유형 | `scope_type` |
| 사유코드 | `reason_code` | 방향 | `direction` |
| 기상세부분류 | `wx_detail` | 비고 | `remarks` |
| MINIT | `minit` | CTOT비고 | `ctot_note` |
| MIT | `mit` | 제한내용요약 | `restriction_summary` |
| 고도속도제한 | `alt_speed_limit` | 품질상태 | `quality_status` |
